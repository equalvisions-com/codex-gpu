...
import type { CpuInfiniteQueryResponse, CpuLogsMeta } from "@/components/cpu-table/cpu-query-options";
...
import type { CpuSearchParamsType } from "@/components/cpu-table/cpu-search-params";
...
export async function GET(req: NextRequest): Promise<Response> {
  try {
    ...
    const search: CpuSearchParamsType = cpuSearchParamsCache.parse(Object.fromEntries(_search));
    ...
    // Only include CPU class rows (GPUs remain stored but are not displayed)
    totalData = totalData.filter((row) => row.class === 'CPU');
    ...
    const res = Response.json({
      ...
      meta: {
        ...
        metadata: {} satisfies CpuLogsMeta,
      },
      prevCursor,
      nextCursor,
    } satisfies CpuInfiniteQueryResponse<CpuColumnSchema[], CpuLogsMeta>, {
      headers: {
        'Cache-Control': 'public, s-maxage=120, stale-while-revalidate=86400',
      },
    });
    logger.info(JSON.stringify({ event: 'cpu-api.page', rowsReturned: rowsOut.length, latencyMs: Date.now() - t1 }));
    return res;
  } catch (error) {
    console.error('Error in CPU pricing API:', error);
    return Response.json(
      {
        error: 'Failed to fetch CPU pricing data',
