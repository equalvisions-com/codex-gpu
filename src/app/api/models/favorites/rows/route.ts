...
async function getFavoriteRowsDirect(
  ...
  try {
    const result = await getCachedFavoriteModelsFiltered(userId, searchParams);
    ...
    const result = await modelsCache.getFavoriteModelsFiltered(userId, searchParams);
    filteredModels = result.data;
    totalCount = result.totalCount;
    filterCount = result.filterCount;
  }
