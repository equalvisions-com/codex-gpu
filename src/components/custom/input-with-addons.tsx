...
>(({ leading, trailing, containerClassName, className, ...props }, ref) => {
  return (
    ...
    >
      {leading ? (
        <div className="py-2 pl-2 bg-background">
          {leading}
        </div>
      ) : null}
      <input
        className={cn(
          "placeholder:text-muted-foreground bg-background w-full rounded-md pl-2 pr-3 py-2 focus:outline-none disabled:cursor-not-allowed disabled:opacity-50",
          className
        )}
        ref={ref}
        {...props}
