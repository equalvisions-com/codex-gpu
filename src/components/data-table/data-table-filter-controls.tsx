...
function SidebarLink({ href, label, isActive }: SidebarItemProps) {
  return (
    <Link
      href={href}
      className={cn(
        "group flex w-full items-center gap-2 rounded-md px-2 py-2 text-sm transition-colors text-foreground hover:bg-muted hover:text-accent-foreground",
        isActive ? "bg-muted text-accent-foreground" : null,
      )}
    >
...
function SidebarAction({ label, onClick }: {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "group flex w-full items-center gap-2 rounded-md px-2 py-2 text-left text-sm transition-colors text-foreground hover:bg-muted hover:text-accent-foreground",
      )}
    >
      <span className="truncate">{label}</span>
    </button>
