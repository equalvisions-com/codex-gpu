...
export function DataTableInfinite<TData, TValue, TMeta, TFavorite = FavoriteKey>({
  ...
  const columnIdList = React.useMemo(
    () =>
      columns
        .map((column) => column.id ?? ("accessorKey" in column ? column.accessorKey : undefined))
        .filter((id): id is string => Boolean(id)),
    [columns],
  );

  const resolvedColumnOrder = React.useMemo(() => {
    if (!columnOrder || columnOrder.length === 0) return columnIdList;

    const seen = new Set<string>();
    const normalizedOrder = columnOrder.filter((id) => {
      const isKnown = columnIdList.includes(id);
      if (isKnown) {
        seen.add(id);
      }
      return isKnown;
