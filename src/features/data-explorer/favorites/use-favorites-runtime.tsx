...
export interface FavoritesRuntimeSnapshot<
  TData,
  TFavoriteKey,
  TLastPage = { meta?: { totalRowCount?: number; filterRowCount?: number } }
> {
  flatData: TData[];
  lastPage?: TLastPage;
...
type FavoritesPage<TData> = {
  data?: TData[];
  meta?: { totalRowCount?: number; filterRowCount?: number };
};

export type FavoritesDataOptions<
  TQueryPage,
  TPageParam = unknown
...
interface UseFavoritesRuntimeParams<
  TData,
  TFavoriteKey,
  TQueryPage extends { data?: TData[]; meta?: { totalRowCount?: number; filterRowCount?: number } } = FavoritesPage<TData>,
  TPageParam = unknown,
  TLastPage extends TQueryPage = TQueryPage
> {
  search?: unknown;
  isActive: boolean;
  ...
  authPending: boolean;
  onStateChange: (
    snapshot: FavoritesRuntimeSnapshot<TData, TFavoriteKey, TLastPage> | null
  ) => void;
  broadcastId: string;
  favoritesDataOptions: FavoritesDataOptions<TQueryPage, TPageParam>;
...
>({
  ...
  syncFavorites,
  stableKey,
}: UseFavoritesRuntimeParams<TData, TFavoriteKey, TQueryPage, TPageParam, TLastPage>) {
  void _search;
  ...
  const {
    ...
    hasNextPage,
    status,
  } = useInfiniteQuery<TQueryPage, Error, InfiniteData<TQueryPage, TPageParam>, readonly unknown[], TPageParam>({
    ...(favoritesDataOptions() as ReturnType<FavoritesDataOptions<TQueryPage, TPageParam>>),
  ...
  const snapshot = React.useMemo<
    FavoritesRuntimeSnapshot<TData, TFavoriteKey, TLastPage>
  >(() => {
    return {
      flatData,
      lastPage: lastPage as TLastPage | undefined,
      totalRowCount,
      filterRowCount,
      totalFetched,
      isFetching,
